...
Give the PDF (e.g. Hoya's lens guide / price list) to a Claude session and
point it at this README. The job is: one row per orderable lens variant
...
| `blank_mm` | no | blank diameter, e.g. `65` or `75` (leave blank for grind — made to size) |
| `sph_min` / `sph_max` | **yes*** | sphere range, signed: `-4.00` and `+4.00` |
...
- If `cyl_max` or `blank_mm` is missing from a row, the lens still shows as
  an option but with an amber "check the guide" warning — the Finder never
  silently assumes.
