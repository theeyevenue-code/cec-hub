...
def recall_chase_sheet():
    ...
    from hub import recall
    path = recall.latest_chase_sheet(_integrations())
    if path is None:
        return ("No phone list has been made yet — press "
                "\"Make a fresh list\" first.", 404)
    return send_from_directory(path.parent, path.name)


@app.route("/api/recall/chasesheet", methods=["POST"])
def recall_chase_refresh():
    """Regenerate Angie's phone list (read-only against Optomate; writes a
    local file). Slow (~1 min) — the UI says so."""
    from hub import recall
    result = recall.refresh_chase_sheet(_integrations())
    if result.get("ok"):
        logger.info(f"Chase sheet regenerated by {_staff_name()}: "
